...
class QueryExecutor:
    def __init__(self, timeout=60, parallel_mode=False, base_mode=True):
        ...
        if self.base_mode:
            ...
            if not self.parallel_mode:
                self.results_file = f'./logs/{config.dbname}_loss{config.delta_weight}_layer{config.num_layers}_learnrate{config.learning_rate}_{config.num_epochs_per_train}per_train.txt'
            else:
                self.results_file = f'./logs/{config.dbname}_loss{config.delta_weight}_layer{config.num_layers}_learnrate{config.learning_rate}_{config.num_epochs_per_train}per_test.txt'
